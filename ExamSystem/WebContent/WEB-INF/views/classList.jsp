				...
				<div class="modal-body">
					<form>
						<div class="form-group">
							<label>班级名称</label> <input type="text" class="form-control"
								id="class_name">
						</div>
						<div class="form-group">
							<label>所属学院</label> 
							<select  class="form-control" id="college_id">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form>
						...
						<div class="form-group">
							<label>所属学院</label> 
							<select  class="form-control" id="savecollege_id">
								  <option value="-1">请选择...</option>
								</select> 
						</div>
					</form>
...
<script type="text/javascript">
	...
	function edit(id) {
		...
		$.ajax({
			...
			success : function(data) {
				$("#class_name").val(data.classs.class_name);
				$.each(data.collegeList, function(index, collegeList) {
		            $("#college_id").append(  //此处向select中循环绑定数据
		    				"<option value="+collegeList.college_id+">" + collegeList.college_name+ "</option>");
		        });
				$("#college_id option[value='"+data.classs.college_id.college_id+"']").attr("selected","selected"); 
			}
		});		
	}
	...
	$("#updateConfirmBtn").click(function() {
		...
		//	contentType:'application/json;charset=utf-8',
			data : {
				...
				college_id : $("#college_id option:selected").val()
	...
	$("#btn_save").click(function() {
		...
		//清楚输入框信息
		$("#saveclass_name").val("");
		$("#savecollege_id option:gt(0)").remove();
		//初始化学院信息
		$.ajax({
			url : 'queryClassById',
			data : {
			...
			success : function(data) {			
				$.each(data.collegeList, function(index, collegeList) {
		            $("#savecollege_id").append(  //此处向select中循环绑定数据
		    				"<option value="+collegeList.college_id+">" + collegeList.college_name+ "</option>");
		        });
				$("#savecollege_id option[value= -1]").attr("selected","selected"); 
			}
		});		
	});
	...
	$("#saveConfirmBtn").click(function() {
		...
		$.ajax({
			...
			datatype:'json',
			data : {
				class_name : $("#saveclass_name").val(),
				college_id : $("#savecollege_id option:selected").val()
