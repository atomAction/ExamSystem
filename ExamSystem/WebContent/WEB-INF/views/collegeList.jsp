...
<body>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title text-center">学院管理</h3>
		</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form>
						...
						<div class="form-group">
							<label>所属学校</label> 
							<select  class="form-control" id="saveschool_id">
								  <option value="-1">请选择...</option>
								</select> 
						</div>
					</form>
...
<script type="text/javascript">
	...
	function edit(id) {
		...
		//清楚输入框信息
		$("#college_name").val("");
		$("#school_id option:gt(0)").remove();
		//初始化用户信息
		$.ajax({
			url : 'queryCollegeById',
			data : {
				'college_id' : id
			},
			success : function(data) {
				$("#college_name").val(data.collegeList.college_name);
				$.each(data.schoolList, function(index, schoolList) {
		            $("#school_id").append(  //此处向select中循环绑定数据
		    				"<option value="+schoolList.school_id+">" + schoolList.school_name+ "</option>");
		        });
				$("#school_id option[value='"+data.collegeList.school_id.school_id+"']").attr("selected","selected"); 
			}
		});		
	}
	...
	$("#updateConfirmBtn").click(function() {
		$("#updateModal").modal('hide');
		$.ajax({
			url : "updateCollegeById",
			method : 'post',
			//contentType:'application/json;charset=utf-8',
			data : {
				college_id : idGlobal,
				college_name : $("#college_name").val(),
				schoolid : $("#school_id option:selected").val()
	...
	$("#btn_save").click(function() {
		...
		//清楚输入框信息
		$("#savecollege_name").val("");
		$("#saveschool_id option:gt(0)").remove();
		//初始化学院信息
		$.ajax({
			url : 'queryCollegeById',
			data : {
			...
			success : function(data) {			
				$.each(data.schoolList, function(index, schoolList) {
		            $("#saveschool_id").append(  //此处向select中循环绑定数据
		    				"<option value="+schoolList.school_id+">" + schoolList.school_name+ "</option>");
		        });
				$("#saveschool_id option[value= -1]").attr("selected","selected"); 
			}
		});		
	});
	...
	$("#saveConfirmBtn").click(function() {
		...
		$.ajax({
			...
			datatype:'json',
			data : {
				college_name : $("#savecollege_name").val(),
				schoolid : $("#saveschool_id option:selected").val()
